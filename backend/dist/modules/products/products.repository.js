...
exports.listRelatedPublicProducts = listRelatedPublicProducts;
exports.getPublicProductCatalogStats = getPublicProductCatalogStats;
const db_1 = require("../../lib/db");
const products_format_1 = require("./products.format");
...
function buildListWhere(filters) {
    ...
    const hasFeatured = filters.featured === true || filters.featured === false
        ? filters.featured
        : undefined;
    const clauses = [];
    const params = [];
    if (!filters.all_status)
...
async function listProducts(filters, limit, offset) {
    const { whereSql, params } = buildListWhere(filters);
    params.push(limit);
    params.push(offset);
...
function detailWhereClause(mode) {
    if (mode === "public") {
        return `p.slug = $1 AND p.status = 'active' AND p.deleted_at IS NULL`;
    }
    return `p.slug = $1 AND p.deleted_at IS NULL`;
}
async function getProductDetailBySlug(slug, mode = "public") {
    const trimmed = slug.trim();
    if (!trimmed)
        return null;
    const head = await (0, db_1.query)(`
      SELECT
        p.id AS uuid,
        p.legacy_key,
        p.slug,
    ...
    const imgRes = await (0, db_1.query)(`
...
async function listRelatedPublicProducts(excludeSlug, groupName, limit) {
